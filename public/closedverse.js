...
(function(a, b) {
    ...
    b.Net = {
        ...
        getErrorFromXHR: function(a) {
			...
			case 500:
				...
				if(a.getResponseHeader('Content-Type') && a.getResponseHeader('Content-Type').indexOf('html') < 0) {
    ...
    b.Form = {
        ...
        __csrf: null,
        _csrf: function() {
            return null === b.Form.__csrf && (b.Form.__csrf = a("#main-body").attr("csrf-token")),
            b.Form.__csrf
        },
		csrftoken: function(a) {
			a._csrf = b.Form._csrf()
			return a
		},
        post: function(c, d, e) {
            d || (d = {}),
...
mode_post = 0;
	...
	b.router.connect("^/login/?$|^/signup/?$", function(c, d, e) {
		function lfinish(b) {
		window.location.href=location.search.split('next=')[1] || '/'
...
//			if($('.setting-nnid').length) {
				...
				$('.color-thing').click(function(a) {
					a.preventDefault();
						$('.color-thing').spectrum({
							...
							flat: true,
							change: function(color) {
								$('.setting-color a.nick-name').attr('style', 'color:' + color);
				...
				$('.default-color').click(function(a) {
					...
					$('.setting-color a.nick-name').attr('style', '');
...
$(document).on('pjax:complete', function(){$('.link-container span').text($('#main-body').attr('time'));NProgress.done()});
}
